...
```console
docker compose exec -w /var/www application composer require --dev squizlabs/php_codesniffer \
...
```console
docker compose exec --user www-data -w /var/www ln -s /var/www/vendor/bin/phpcs /usr/local/bin/phpcs && ln -s /var/www/vendor/bin/phpcbf /usr/local/bin/phpcbf
```

To run *PHP Code Sniffer* use command:

```console
docker compose exec -w /var/www application /var/www/vendor/bin/phpcs
```
...
```console
docker compose exec -w /var/www application composer update
```
